...
export const generateCommand = new EasyCLICommand<{}, DesignSystemGlobalFlags>(
  ...
  async (flags: DesignSystemGlobalFlags, theme: any) => {
    ...
    logger.success(
      `🏗️  Generating consolidated MCP file from context files...\n`,
    );
    ...
    try {
      ...
      if (storyFiles.length === 0) {
        ...
        logger.info(
          "Run 'design-system-mcp scan' to check your configuration.",
        );
        return;
      }

      logger.info(
        `📁 Looking for existing context files for ${storyFiles.length} story files...`,
      );
      ...
      for (const storyFile of storyFiles) {
        ...
        if (existsSync(contextFilePath)) {
          try {
            ...
            if (
              contextContent.components &&
              Array.isArray(contextContent.components)
            ) {
              ...
              const components = contextContent.components.map((comp: any) => ({
                ...
                slots: comp.slots || [],
                variants: [],
                examples: comp.codeExamples
                  ? [{ title: 'basic', code: comp.codeExamples.basic }]
                  : [],
                storyFile,
          ...
          logger.info(
            `     Run 'design-system-mcp parse' to generate context files first.`,
          );
      ...
      logger.info(
        `📊 Categories: ${Object.keys(mcpOutput.componentIndex.byCategory).length}`,
      );
      logger.info(
